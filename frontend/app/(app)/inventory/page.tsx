...
import { CheckCircle2, Pencil, RotateCcw, Search, X } from "lucide-react";
...
import { useApplyInventory, useInventoryItems } from "@/lib/hooks/useStock";
import { ItemType, Permission } from "@/lib/types/enums";
import type { InventoryItemRead, InventoryResult } from "@/lib/types/stock";
import {
 ...
 * с проверкой, что остаток не изменился, пока его редактировали.
 */
...
const qtyFormat = new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 3 });
const inputFormat = new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 3, useGrouping: false });
const fmtQty = (v: number) => qtyFormat.format(v);
...
/** «+20 шт · приход» / «−5 шт · расход». */
function DeltaPill({ delta, unit, withKind = false }: { delta: number; unit: string; withKind?: boolean }) {
  const isIn = delta > 0;
  return (
    <span
      className={clsx(
    ...
    <div className={clsx("flex flex-col gap-4", editing && "pb-28 lg:pb-24")}>
      ...
      <div className="glass flex flex-wrap items-center gap-3 rounded-2xl p-3">
        ...
        {editing ? (
          ...
          <Button onClick={startEditing} disabled={inventory.isLoading}>
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-success/20 bg-success-bg px-4 py-3 text-[13px] text-text">
          ...
          <div className="min-w-0 flex-1">
            {result.changes.length === 0 ? (
              ...
              <>
                <span className="font-semibold">Проведено.</span> Изменено позиций: {result.changes.length}, движений
                склада: {result.movements_created}. Их видно в{" "}
                <Link href="/stock" className="font-semibold text-primary hover:underline">
                  «Остатки → Движения»
                </Link>{" "}
                с пометкой «Инвентаризация».
              </>
            )}
          </div>
          <button
      ...
      {/* ===== Плавающая панель сохранения (режим правки) ===== */}
      {editing && (
          ...
          {changes.map((c) => (
            <div key={keyOf(c.row)} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3.5 py-2.5">
              ...
              <span className="text-[12.5px] tabular-nums text-muted">
                {fmtQty(c.before)} → <b className="text-text">{fmtQty(c.after)}</b> {c.row.unit}
              </span>
